        ...
        $.ajax({
            ...
            success: function (data) {
               ...
               let avg = '';
                ...
                const hs = document.getElementById('highestS');
                const ls =document.getElementById('lowestS');
           
               
                hs.innerText = `the highest stock value is ${highestStockvalue} on ${date}`;
                ls.innerText = `the lowest stock value is ${lowest} on ${date2}`;
...
function drawChart() {
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            var arrSales2 = [['Date' ,'highest rate']];  

            $.each(data, function (index, value) {
                if(start <= value.date && end >=value.date)
                {
                    let avg = (value.open + value.close)/2;
                arrSales2.push([value.date,avg ]);
                }
            });  
  var options = {'title':'My Average Day', 
                 'subtitle': 'the highest stock value is',
                 'width':550, 
                 'height':400};
  var figures2 = google.visualization.arrayToDataTable(arrSales2)

  var chart = new google.visualization.PieChart(document.getElementById('chart3'));
  chart.draw(figures2, google.charts.Line.convertOptions(options));
   },
         error: function (XMLHttpRequest, textStatus, errorThrown) {
          alert('Got an Error');
     }
